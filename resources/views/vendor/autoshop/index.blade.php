...
@section('page-title', 'Autoshop — My Bases')
...
<div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Autoshop</h1>
            <p class="text-sm text-gray-500 mt-1">Upload CSV files and sell fullz records to buyers.</p>
        </div>
        <a href="{{ route('vendor.autoshop.create') }}"
    ...
    @if($bases->isEmpty())
        <div class="bg-white border border-gray-200 rounded-xl p-12 text-center">
            <p class="text-gray-500 mb-4">You have not uploaded any bases yet.</p>
            <a href="{{ route('vendor.autoshop.create') }}"
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium text-gray-900">
                            <a href="{{ route('vendor.autoshop.show', $base) }}" class="hover:text-purple-700">
                                {{ $base->name }}
                            </a>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('vendor.autoshop.show', $base) }}"
                                   class="text-xs text-purple-700 hover:underline">View</a>

                                <form action="{{ route('vendor.autoshop.toggle', $base) }}" method="POST" class="inline">
                                    @csrf
                                ...
                                <form action="{{ route('vendor.autoshop.destroy', $base) }}" method="POST" class="inline"
                                      onsubmit="return confirm('Delete this base and all records?')">
                                    @csrf
                                    @method('DELETE')
