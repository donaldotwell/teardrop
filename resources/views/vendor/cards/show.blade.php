...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="mb-5">
        <a href="{{ route('vendor.cards.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Cards</a>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                <form action="{{ route('vendor.cards.toggle', $base) }}" method="POST">
                    @csrf
                    <button type="submit"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <form action="{{ route('vendor.cards.update', $base) }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Price per Card (USD)</label>
                    <div class="flex items-center gap-2">
                ...
                <div class="mb-4">
                    <label class="flex items-start gap-2 cursor-pointer">
                        ...
                        <span class="text-xs text-gray-600">
                            Also reprice existing unsold cards in this base
                        </span>
                    </label>
                </div>
                <button type="submit"
        ...
        {{-- Upload more --}}
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            <h2 class="text-sm font-semibold text-gray-800 mb-4">Add More Cards</h2>
            <form action="{{ route('vendor.cards.upload', $base) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Card File</label>
                    <input type="file" name="file" accept=".csv,.txt" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-500">
                    <p class="text-xs text-gray-400 mt-1">Same pipe-delimited format. Max 10 MB.</p>
                </div>
                <p class="text-xs text-gray-500 mb-4">Cards appended at current price (${{ number_format($base->price_usd, 2) }}/card).</p>
                <button type="submit"
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-800">Cards</h2>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="{{ $card->status === 'sold' ? 'bg-gray-50 text-gray-400' : '' }}">
                        <td class="px-3 py-2 font-mono">{{ $card->card_number }}</td>
                        <td class="px-3 py-2 font-mono">{{ $card->exp_month }}/{{ $card->exp_year }}</td>
                        <td class="px-3 py-2 font-mono">{{ $card->cvv }}</td>
                        <td class="px-3 py-2 font-medium">{{ $card->cardholder_name }}</td>
                        ...
                        <td class="px-3 py-2 text-gray-400">{{ $card->sold_at?->format('M d') ?? '—' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
