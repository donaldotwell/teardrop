...
<div class="space-y-6">
    ...
    @if($featuredListings->count() > 0)
        <div class="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl shadow-sm border-2 border-yellow-300 p-6">
            <div class="flex items-center gap-2 mb-4">
                <span class="text-2xl text-yellow-600">★</span>
                <h2 class="text-xl font-bold text-yellow-800">Featured Listings</h2>
                <span class="bg-yellow-200 text-yellow-800 text-xs px-2 py-1 rounded-full">{{ $featuredListings->count() }}</span>
            </div>
    ...
    @if($regularListings->count() > 0)
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-2 mb-4">
                <h2 class="text-xl font-bold text-gray-800">Regular Listings</h2>
                <span class="bg-gray-200 text-gray-800 text-xs px-2 py-1 rounded-full">{{ $regularListings->total() }}</span>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            {{ $regularListings->links() }}
        </div>
    @endif

    <!-- Empty State -->
    @if($featuredListings->count() === 0 && $regularListings->count() === 0)
