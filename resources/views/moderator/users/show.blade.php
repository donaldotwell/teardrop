...
@section('content')
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        {{-- PGP Key --}}
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">PGP Public Key</h3>
            @if($user->pgp_pub_key)
                <div class="flex items-center justify-between gap-4">
                    <p class="text-sm text-gray-600">User has a PGP key on file.</p>
                    <x-modal
                        id="pgpModal"
                        title="PGP Public Key — {{ $user->username_pub }}"
                        triggerText="View PGP Key"
                        triggerClass="flex-shrink-0 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors"
                    >
                        <div class="space-y-3">
                            <p class="text-sm text-gray-600">Public key for {{ $user->username_pub }}</p>
                            <textarea
                                class="w-full h-64 p-3 border border-gray-300 rounded-lg font-mono text-xs focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                readonly
                            >{{ $user->pgp_pub_key }}</textarea>
                        </div>
                        <x-slot:footer>
                            <label for="pgpModal" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm cursor-pointer">
                                Close
                            </label>
                        </x-slot:footer>
                    </x-modal>
            ...
            @else
                <p class="text-sm text-gray-500">No PGP key on file.</p>
            @endif
        </div>
