...
@php
    $hasFilters     = request()->anyFilled(['vendor_id','base_id','state','name','city','zip','gender','phone','price_min','price_max']);
    $advancedActive = request()->anyFilled(['city','zip','gender','phone','price_min','price_max','sort']);
@endphp

{{-- Filter card --}}
<form method="GET" action="{{ route('autoshop.fullz.index') }}" class="mb-5">
    <div class="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
        ...
        {{-- Advanced filters (no-JS collapsible via <details>) --}}
            ...
            <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <select name="phone"
                            class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-teal-500 bg-white">
                        <option value="">Any</option>
                        <option value="yes" {{ request('phone') === 'yes' ? 'selected' : '' }}>Has phone</option>
                        <option value="no"  {{ request('phone') === 'no'  ? 'selected' : '' }}>No phone</option>
                    </select>
                </div>
                <div>
...
<form action="{{ route('autoshop.fullz.purchase') }}" method="POST">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
        <div class="px-4 py-3 bg-teal-50 border-b border-teal-100 text-xs text-teal-800">
            Select records to buy. SSN and DOB are revealed after purchase.
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-center font-semibold text-gray-700">Phone</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-teal-50 transition-colors">
                        ...
                        <td class="px-3 py-2 text-xs text-gray-500 whitespace-nowrap">
                            <a href="{{ route('autoshop.fullz.index', ['vendor_id' => $record->base_vendor_id, 'base_id' => $record->base_id]) }}"
                               class="hover:text-teal-700">{{ $record->base_name }}</a>
                        </td>
                        <td class="px-3 py-2 font-medium text-gray-900 whitespace-nowrap">{{ $record->name }}</td>
                        <td class="px-3 py-2 text-gray-600 text-xs">{{ $record->gender ? ucfirst($record->gender) : '—' }}</td>
                        <td class="px-3 py-2 text-gray-600 text-xs">{{ $record->state ?? '—' }}</td>
                        <td class="px-3 py-2 text-gray-600 text-xs">{{ $record->city ?? '—' }}</td>
                        <td class="px-3 py-2 text-center">
                            @if($record->phone_no)
