...
@section('content')
    ...
    {{-- ── Filter bar (top) ─────────────────────────────────────────── --}}
    <form method="GET" action="{{ route('autoshop.index') }}" class="mb-6">
        <div class="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                @if(request()->anyFilled(['base_id','vendor_id','state','gender','price_min','price_max','sort']))
                    <a href="{{ route('autoshop.index') }}" class="text-xs text-amber-700 hover:underline">Clear all</a>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 items-end">
                ...
                <div>
                    ...
                    <select name="base_id"
                            class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 bg-white">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="price_min" value="{{ request('price_min') }}"
                               ...
                               class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
                ...
                <div>
                    <button type="submit"
                            class="w-full py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm">
                        Apply Filters
                    </button>
                </div>
            </div>
    ...
    <div class="min-w-0">
        ...
        <form action="{{ route('autoshop.purchase') }}" method="POST">
            ...
            <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
                {{-- Info bar --}}
                <div class="px-4 py-3 bg-amber-50 border-b border-amber-100 text-xs text-amber-800">
            ...
            <div class="bg-white border border-gray-200 rounded-xl px-5 py-4">
                <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <button type="submit"
                            class="sm:ml-auto w-full sm:w-auto px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm rounded-lg transition-colors shadow-sm">
                        Purchase Selected
                    </button>
                </div>
