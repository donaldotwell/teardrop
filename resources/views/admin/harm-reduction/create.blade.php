...
@section('title', 'Create Harm Reduction Content')
...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {{-- Header --}}
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Create New Harm Reduction Content</h1>
            <p class="mt-1 text-sm text-gray-600">Add new safety information or harm reduction tip</p>
        </div>

        {{-- Form --}}
        <div class="bg-white shadow overflow-hidden rounded-lg">
            <form action="{{ route('admin.harm-reduction.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="6" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 @error('content') border-red-500 @enderror">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', 1) }}" min="0" required
