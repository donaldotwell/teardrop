...
@section('page-title', 'BTC Wallet: ' . $btcWallet->name)
@section('page-heading', 'BTC Wallet: ' . $btcWallet->name)
@section('page-description')
    @if($btcWallet->user)
        Owner: {{ $btcWallet->user->username_pub }}
    @else
...
@section('breadcrumbs')
    ...
    <span class="text-gray-900 font-medium">{{ $btcWallet->name }}</span>
@endsection
...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div class="text-2xl font-bold text-gray-900">{{ number_format($btcWallet->balance, 8) }} BTC</div>
            <div class="text-sm text-gray-500 mt-1">~ ${{ number_format(convert_crypto_to_usd($btcWallet->balance, 'btc'), 2) }} USD</div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-mono text-gray-900">{{ number_format($btcWallet->total_received, 8) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Total Sent</span>
                    <span class="font-mono text-gray-900">{{ number_format($btcWallet->total_sent, 8) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Address Index</span>
                    <span class="font-mono text-gray-900">{{ $btcWallet->address_index }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="text-gray-900">{{ $btcWallet->id }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Status</span>
                    @if($btcWallet->is_active)
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-gray-900">{{ $btcWallet->created_at->format('Y-m-d H:i') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-6">
        @if($btcWallet->balance > 0)
            <a href="{{ route('admin.wallets.btc.transfer', $btcWallet) }}"
    ...
    {{-- Addresses --}}
    @if($btcWallet->addresses->count() > 0)
        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden mb-6">
            <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
                <h3 class="font-semibold text-gray-900">Addresses ({{ $btcWallet->addresses->count() }})</h3>
            </div>
            <div class="divide-y divide-gray-100">
                @foreach($btcWallet->addresses->sortByDesc('address_index')->take(10) as $address)
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 text-center">
                            @if($tx->status === 'confirmed')
                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded bg-green-100 text-green-700">Confirmed</span>
                            @elseif($tx->status === 'pending')
                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded bg-yellow-100 text-yellow-700">Pending</span>
                            @else
                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded bg-red-100 text-red-700">{{ ucfirst($tx->status) }}</span>
                            @endif
                        </td>
