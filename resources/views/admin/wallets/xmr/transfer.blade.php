...
@section('page-title', 'Transfer XMR')
@section('page-heading', 'Transfer XMR')
@section('page-description', 'Send Monero from wallet: ' . $xmrWallet->name)
...
@section('content')
    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 mb-6">
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <span class="font-medium text-gray-900 ml-1">{{ $xmrWallet->name }}</span>
                </div>
                <div>
                    <span class="text-gray-600">Owner:</span>
                    <span class="font-medium text-gray-900 ml-1">{{ $xmrWallet->user ? $xmrWallet->user->username_pub : 'Escrow' }}</span>
                </div>
                <div>
                    <span class="text-gray-600">Unlocked Balance:</span>
                    <span class="font-mono font-bold text-gray-900 ml-1">{{ number_format($xmrWallet->unlocked_balance, 8) }} XMR</span>
                </div>
                <div>
                    <span class="text-gray-600">~ USD:</span>
                    <span class="font-medium text-gray-900 ml-1">${{ number_format(convert_crypto_to_usd($xmrWallet->unlocked_balance, 'xmr'), 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <form action="{{ route('admin.wallets.xmr.transfer-initiate', $xmrWallet) }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           id="address"
                           value="{{ old('address') }}"
                           class="w-full px-3 py-2 border @error('address') border-red-400 @else border-gray-300 @enderror rounded focus:outline-none focus:border-amber-500 font-mono text-xs"
                           placeholder="Monero address (95+ characters)"
                           required>
                    @error('address')
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           required>
                    <p class="text-xs text-gray-500 mt-1">Max: {{ number_format($xmrWallet->unlocked_balance, 12) }} XMR (network fee will be deducted additionally)</p>
                    @error('amount')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
